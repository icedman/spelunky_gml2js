...
if (global.levelType == 1 && !global.madeBlackMarket && global.genBlackMarket) {
...
if (global.levelType == 3 && rand(1, global.probSacPit) == 1) {
  while (n == roomX) {
    n = rand(0, 3);
  }
...
while (roomY < 4) {
  ...
  if (n < 3 || n > 5) roomX -= 1; // move left
  else if (n == 3 || n == 4) roomX += 1; // move right
  // move down
  else {
    roomY += 1;
    d = true;
...
if (global.levelType == 2) {
  if (!global.madeMoai) {
    // Moai
    if (global.currLevel == 9 && rand(1, 4) == 1) global.madeMoai = true;
    else if (global.currLevel == 10 && rand(1, 3) == 1) global.madeMoai = true;
    else if (global.currLevel == 11 && rand(1, 2) == 1) global.madeMoai = true;
...
// shop
if (
  rand(1, global.currLevel) <= 2 &&
  global.currLevel > 1 &&
  !global.madeBlackMarket
) {
  i = 0;
  for (k = 0; k < 4; k += 1) {
    for (j = 0; j < 4; j += 1) {
