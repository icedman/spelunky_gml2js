...
// scrInitLevel()
//
// Calls scrLevelGen(), scrRoomGen*(), && scrEntityGen() to build level.
//
...
global.levelType = 0;
//global.currLevel = 16;
if (global.currLevel > 4 && global.currLevel < 9) global.levelType = 1;
if (global.currLevel > 8 && global.currLevel < 13) global.levelType = 2;
if (global.currLevel > 12 && global.currLevel < 16) global.levelType = 3;
  ...
  for (j = 0; j <= k; j += 1) {
    if (!isLevel()) {
      ...
      if (i * 16 == 0 || i * 16 == 656 || j * 16 == 0) {
        ...
        obj.invincible = true;
        if (!global.cityOfGold) obj.sprite_index = sTemple;
      }
    } else if (global.lake) {
    ...
    } else if (i * 16 == 0 || i * 16 == 656 || j * 16 == 0 || j * 16 >= 528) {
...
//if (!global.hadDarkLevel && global.currLevel != 0 && global.levelType != 2 && global.currLevel != 16 && rand(1,1) == 1)
if (
  !global.hadDarkLevel &&
  !global.noDarkLevel &&
  global.currLevel != 0 &&
...
if (!global.madeUdjatEye) {
  if (global.currLevel == 2 && rand(1, 3) == 1) global.genUdjatEye = true;
  else if (global.currLevel == 3 && rand(1, 2) == 1) global.genUdjatEye = true;
  else if (global.currLevel == 4) global.genUdjatEye = true;
}

global.genMarketEntrance = false;
if (!global.madeMarketEntrance) {
  if (global.currLevel == 5 && rand(1, 3) == 1) global.genMarketEntrance = true;
  else if (global.currLevel == 6 && rand(1, 2) == 1)
    global.genMarketEntrance = true;
...
if (!isRoom('rTutorial') && !isRoom('rLoadLevel')) scrEntityGen();

if (instance_exists(oEntrance) && !global.customLevel) {
...
if (global.graphicsHigh) {
  for (r = 0; r < c; r++) {
    ...
    if (global.levelType == 1 && rand(1, 3) < 3)
      tile_add(
        bgExtrasLush,
        32 * rand(0, 1),
        0,
    ...
    else if (global.levelType == 2 && rand(1, 3) < 3)
      tile_add(
        bgExtrasIce,
        32 * rand(0, 1),
        0,
    ...
    else if (global.levelType == 3 && rand(1, 3) < 3)
      tile_add(
        bgExtrasTemple,
        32 * rand(0, 1),
        0,
