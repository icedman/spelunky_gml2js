...
[instances_of(oCharacter)].forEach(($) => { with($)
...
[instances_of(oMovingSolid)].forEach(($) => { with($)
  ...
  if(solidIsNearPlayers)
  {
    ...
    if (xVelInteger>0)
    {
      ...
      for(x=x;x<mstXPrev+xVelInteger;x+=1)
      {
        for(i=0;i<oGame.players_length;i+=1)
        {
          if(viscidTop && isCollisionCharacterTop(1,oGame.players[i]) && ((oGame.players[i]).viscidMovementOk=1 || (oGame.players[i]).viscidMovementOk=2))
          {
[instances_of(oGame)].forEach(($) => { with($)
...
collision=isCollisionLeft(1)})
    ...
    if (yVelInteger>0)
    {
      ...
      for(y=y;y<mstYPrev+yVelInteger;y+=1)
      {
        for(i=0;i<oGame.players_length;i+=1)
        {
          if (viscidTop && ) isCollisionCharacterTop(2,oGame.players[i])
          {
...
[instances_of(oMoveableSolid)].forEach(($) => { with($)

{
        ...
        y > view_yview[0]-16 && y < view_yview[0] + view_hview[0]))
    {
        ...
        //is there a (precise) collision
            if (place_meeting(x, y+1, oSolid)) // || isCollisionCharacterBottom(2))
            {
