  ...
  if (global.levelType == 0) {
      ...
      with ($) {
        if (!isInShop(x, y) && y > 16) {
          ...
          if (
            ...
            scrGetRoomY(y - 16) != global.startRoomY
          ) {
            if (
              y < room_height - 64 &&
              ...
              } else if (global.darkLevel && rand(1, 60) == 1)
                instance_create(x, y + 16, oLamp);
              else if (global.darkLevel && rand(1, 40) == 1)
                instance_create(x, y + 16, oScarab);
              else if (rand(1, 60) == 1) instance_create(x, y + 16, oBat);
              else if (rand(1, 80) == 1)
                instance_create(x, y + 16, oSpiderHang);
            }
    ...
    instances_of(oSolid).forEach(($) => {
      with ($) {
        ...
        if (rand(1, 100) == 1 && !collision_point(x, y - 16, oSolid, 0, 0))
          tile_add(bgTrees, 0, 0, 16, 48, x, y - 32, 9005);

        if (!isInShop(x, y)) {
          if (
            y > 32 &&
            collision_point(x, y - 16, oSolid, 0, 0) &&
            global.genMarketEntrance &&
            !global.madeMarketEntrance
          ) {
          ...
          if (
            ...
            if (
              ...
              if (global.darkLevel && rand(1, 40) == 1)
                instance_create(x, y + 16, oScarab);
            ...
            if (
              ...
              if (global.cemetary) {
                ...
                else if (rand(1, 160) == 1)
                  instance_create(x, y - 16, oVampire);
              } else if (!collision_point(x, y - 16, oWater, 0, 0)) {
                if (global.blackMarket && y % 128 == 0)
                  n = 0; // to prevent mantraps from spawning near shopkeepers in black market
                else n = 1;
        ...
        if (!isInShop(x, y)) {
          ...
          if (
            ...
            if (
              rand(1, 10) == 1 &&
              sprite_index == sDark &&
              !collision_rectangle(x, y - 64, x + 15, y - 1, oSolid, 0, 0) &&
              distance_to_object(oExit) > 64
            )
              instance_create(x, y - 16, oSpringTrap);
            else if (
              rand(1, 20) == 1 &&
              point_distance(x, y, oEntrance.x, oEntrance.y) > 64
            )
              instance_create(x, y - 16, oYeti);
          }
    ...
    instances_of(oSolid).forEach(($) => {
      with ($) {
        // bg
        if (rand(1, 100) == 1 && !collision_point(x, y - 16, oSolid, 0, 0))
          tile_add(bgStatues, 0, 0, 16, 48, x, y - 32, 9005);
        ...
        if (!isInShop(x, y)) {
          ...
          if (
            y > 32 &&
            !collision_point(x, y - 16, oSolid, 0, 0) &&
            global.genGoldEntrance &&
            !global.madeGoldEntrance
          ) {
